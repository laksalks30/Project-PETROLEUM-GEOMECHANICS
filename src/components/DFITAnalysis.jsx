  ...
  return (
    <div className="card h-full flex flex-col" id="panel-dfit" style={{ background: '#0a1428', border: '1px solid rgba(30,41,59,0.8)' }}>
      ...
      <div className="flex items-center gap-2" style={{
        ...
        <span style={{ fontSize: 11, fontWeight: 800, color: '#f8fafc', letterSpacing: '0.08em' }}>DFIT ANALYSIS — PRESSURE DECLINE</span>
      </div>
        ...
        <div className="flex-1 min-h-[140px] w-full" style={{ paddingRight: 10 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 10, right: 30, left: -15, bottom: 0 }}>
              ...
              <XAxis dataKey="time_min" tick={{ fill: '#64748b', fontSize: 11, fontWeight: 600 }} tickLine={false} axisLine={{ stroke: '#334155' }} label={{ value: 'TIME (MIN)', position: 'insideBottom', fill: '#475569', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', dy: 10 }} />
              <YAxis domain={[4000, 9500]} tick={{ fill: '#64748b', fontSize: 11, fontWeight: 600 }} tickLine={false} axisLine={false} tickFormatter={v => `${(v/1000).toFixed(0)}k`} />
              <Tooltip content={<CustomTooltip />} cursor={{ stroke: 'rgba(56,189,248,0.2)', strokeWidth: 2 }} />
              <ReferenceLine y={dfit.breakdown_psi}         stroke="#ef4444" strokeDasharray="4 4" strokeOpacity={0.6} label={{ value: `BD ${dfit.breakdown_psi}`, fill: '#ef4444', fontSize: 11, position: 'right', fontWeight: 700 }} />
              <ReferenceLine y={dfit.ISIP_psi}             stroke="#f59e0b" strokeDasharray="4 4" strokeOpacity={0.6} label={{ value: `ISIP ${dfit.ISIP_psi}`, fill: '#f59e0b', fontSize: 11, position: 'right', fontWeight: 700 }} />
              <ReferenceLine y={dfit.closure_psi}          stroke="#22c55e" strokeDasharray="4 4" strokeOpacity={0.6} label={{ value: `Closure ${dfit.closure_psi}`, fill: '#22c55e', fontSize: 11, position: 'right', fontWeight: 700 }} />
              <ReferenceLine y={dfit.reservoir_pressure_psi} stroke="#3b82f6" strokeDasharray="4 4" strokeOpacity={0.6} label={{ value: `Pp ${dfit.reservoir_pressure_psi}`, fill: '#3b82f6', fontSize: 11, position: 'right', fontWeight: 700 }} />
...
function RefItem({ label, value, color }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span style={{ fontSize: 11, color: '#f8fafc', fontWeight: 800 }}>{value?.toLocaleString()}</span>
    </div>
  )
}
...
function MetricBox({ label, value, color, highlight }) {
  return (
    <div className="flex flex-col items-center justify-center p-2 rounded-lg" style={{ 
      ...
      <span style={{ fontSize: 10, color: highlight ? '#22c55e' : '#64748b', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700 }}>{label}</span>
      <span style={{ fontSize: 14.5, color: highlight ? '#22c55e' : '#f8fafc', fontWeight: 800, marginTop: 2, letterSpacing: '0.02em' }}>{value}</span>
    </div>
  )
}
  ...
  return (
    <div className="card h-full animate-pulse flex flex-col" style={{ background: '#0a1428', border: '1px solid rgba(30,41,59,0.8)' }}>
      ...
      <div className="flex-1 flex items-center justify-center">
        <span style={{ color: '#334155', fontSize: 11, fontWeight: 700, letterSpacing: '0.1em' }}>LOADING DFIT...</span>
      </div>
    </div>
  )
}
