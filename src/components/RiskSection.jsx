    ...
    <div className="grid gap-2 h-full" style={{ gridTemplateColumns: '220px 230px 1.2fr 1.8fr' }}>
      ...
      <div className="card flex flex-col overflow-hidden" id="panel-risk-matrix" style={{ background: '#0a1428', border: '1px solid rgba(30,41,59,0.8)' }}>
        <div className="flex items-center gap-2" style={{ padding: '8px 12px', background: 'linear-gradient(90deg, rgba(56,189,248,0.1) 0%, transparent 100%)', borderBottom: '1px solid rgba(56,189,248,0.15)', borderLeft: '2px solid #38bdf8' }}>
          <span style={{ fontSize: 10, fontWeight: 800, color: '#f8fafc', letterSpacing: '0.08em' }}>RISK MATRIX</span>
        </div>
        <div className="p-2 flex flex-col gap-2 flex-1">
          <RiskMatrix risks={risks} />
          <div className="mt-1 flex flex-col items-center">
            <div style={{ fontSize: 8, color: '#64748b', marginBottom: 6, fontWeight: 800, letterSpacing: '0.05em' }}>RISK LEVEL GUIDE</div>
      ...
      <div className="card flex flex-col overflow-hidden" id="panel-highlighted-risks" style={{ background: '#0a1428', border: '1px solid rgba(30,41,59,0.8)' }}>
        <div className="flex items-center gap-2" style={{ padding: '8px 12px', background: 'linear-gradient(90deg, rgba(56,189,248,0.1) 0%, transparent 100%)', borderBottom: '1px solid rgba(56,189,248,0.15)', borderLeft: '2px solid #38bdf8' }}>
          <span style={{ fontSize: 10, fontWeight: 800, color: '#f8fafc', letterSpacing: '0.08em' }}>HIGHLIGHTED RISKS</span>
        </div>
          ...
          {(risk.highlighted_risks || []).map((r, i) => (
            <div key={i} className="flex items-center justify-between px-3 py-2.5 rounded-lg" style={{ background: '#070f22', border: '1px solid rgba(30,41,59,0.8)' }}>
              <div className="flex gap-2 items-center">
                <div className="w-5 h-5 rounded-full flex items-center justify-center bg-blue-900/40 text-blue-400 border border-blue-500/30" style={{ fontSize: 9, fontWeight: 800 }}>{i + 1}</div>
                <span style={{ fontSize: 10, color: '#e2e8f0', fontWeight: 600 }}>{r.name}</span>
              </div>
      ...
      <div className="card flex flex-col overflow-hidden" id="panel-overall-assessment" style={{ background: '#0a1428', border: '1px solid rgba(30,41,59,0.8)' }}>
        <div className="flex items-center gap-2" style={{ padding: '8px 12px', background: 'linear-gradient(90deg, rgba(56,189,248,0.1) 0%, transparent 100%)', borderBottom: '1px solid rgba(56,189,248,0.15)', borderLeft: '2px solid #38bdf8' }}>
          <span style={{ fontSize: 10, fontWeight: 800, color: '#f8fafc', letterSpacing: '0.08em' }}>OVERALL ASSESSMENT</span>
        </div>
        <div className="p-3 flex flex-col gap-2 flex-1 justify-center">
          <div className="rounded-xl p-5 text-center flex flex-col justify-center h-full relative overflow-hidden" style={{
            ...
            <div className="relative z-10 mt-2" style={{ fontSize: 10, color: '#94a3b8', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 700 }}>
      ...
      <div className="card flex flex-col min-w-0 overflow-hidden" id="panel-recommendations" style={{ background: '#0a1428', border: '1px solid rgba(30,41,59,0.8)' }}>
        <div className="flex items-center gap-2" style={{ padding: '8px 12px', background: 'linear-gradient(90deg, rgba(56,189,248,0.1) 0%, transparent 100%)', borderBottom: '1px solid rgba(56,189,248,0.15)', borderLeft: '2px solid #38bdf8' }}>
          <span style={{ fontSize: 10, fontWeight: 800, color: '#f8fafc', letterSpacing: '0.08em' }}>RECOMMENDATIONS</span>
        </div>
        <div className="p-2.5 flex flex-col gap-2.5 flex-1 overflow-y-auto custom-scrollbar">
          {recs.map((rec, i) => (
            <div key={i} className="flex items-start gap-3 p-3 rounded-lg" style={{ background: 'rgba(56,189,248,0.05)', border: '1px solid rgba(56,189,248,0.15)', borderLeft: '3px solid #38bdf8' }}>
              ...
              <span style={{ fontSize: 10, color: '#e2e8f0', lineHeight: 1.6, letterSpacing: '0.02em', fontWeight: 500 }}>{rec}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="flex items-center justify-center p-2 bg-[#070f22] rounded-lg border border-[#1e293b]">
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <div style={{ fontSize: 8, color: '#64748b', writingMode: 'vertical-rl', transform: 'rotate(180deg)', letterSpacing: '0.15em', fontWeight: 800 }}>
          CONSEQUENCE
        </div>
        <div>
          {grid.map(({ c, cells }) => (
            <div key={c} style={{ display: 'flex', gap: 4, marginBottom: 4, alignItems: 'center' }}>
              <span style={{ fontSize: 8, color: '#64748b', width: 10, textAlign: 'right', marginRight: 4, fontWeight: 800 }}>{c}</span>
                ...
                >
                  {match ? (
                    <div style={{
                      ...
                      color: 'white', fontSize: 10, fontWeight: 900, boxShadow: '0 2px 6px rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.1)'
                    }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginLeft: 18, marginTop: 4 }}>
            {[1, 2, 3, 4, 5].map(l => (
              <div key={l} style={{ width: 30, textAlign: 'center', fontSize: 8, color: '#64748b', fontWeight: 800 }}>{l}</div>
            ))}
          </div>
          <div style={{ textAlign: 'center', marginLeft: 18, fontSize: 8, color: '#64748b', marginTop: 6, letterSpacing: '0.15em', fontWeight: 800 }}>
            LIKELIHOOD
          </div>
        </div>
      </div>
...
function RiskBadge({ level }) {
  ...
  return (
    <span style={{
      ...
      padding: '3px 8px', borderRadius: 4, fontSize: 8, fontWeight: 800, letterSpacing: '0.05em'
    }}>
      {c.label}
    </span>
  )
...
function LegendDot({ color, label }) {
  return (
    <div className="flex items-center gap-1.5 px-2 py-1 rounded" style={{ background: 'rgba(255,255,255,0.02)' }}>
      ...
      <span style={{ fontSize: 8, color: '#94a3b8', letterSpacing: '0.05em', fontWeight: 800 }}>{label}</span>
    </div>
  )
}
...
function IconBox({ icon: Icon, label, color }) {
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <span style={{ fontSize: 9, color: '#e2e8f0', textAlign: 'center', maxWidth: 65, letterSpacing: '0.05em', fontWeight: 700 }}>{label}</span>
    </div>
  )
}
