...
function RiskBadge({ level }) {
  ...
  return (
    <span style={{
      ...
      padding: '2px 6px', borderRadius: 4, fontSize: 8, fontWeight: 800, letterSpacing: '0.05em'
    }}>
      {c.label}
    </span>
  )
  ...
  return (
    <div className="card h-full flex flex-col overflow-hidden" id="panel-containment" style={{ background: '#0a1428', border: '1px solid rgba(30,41,59,0.8)' }}>
      ...
      <div className="flex items-center gap-2" style={{
        ...
        <span style={{ fontSize: 10, fontWeight: 800, color: '#f8fafc', letterSpacing: '0.08em' }}>CONTAINMENT ANALYSIS</span>
      </div>
        ...
        {(upper_breach || lower_breach) && (
          <div className="flex flex-col gap-1 rounded-lg px-3 py-2" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderLeft: '3px solid #ef4444' }}>
            <div className="flex items-center gap-1.5">
              ...
              <span style={{ fontSize: 9, fontWeight: 800, color: '#ef4444', letterSpacing: '0.05em' }}>NET PRESSURE ({Pnet_psi?.toLocaleString()} PSI) EXCEEDS CONTRASTS</span>
            </div>
            <div style={{ fontSize: 10, color: '#f8fafc', fontWeight: 700 }}>HIGH RISK OF HEIGHT GROWTH</div>
          </div>
        )}

        <div className="flex items-center justify-between bg-[#070f22] px-3 py-2.5 rounded-lg border border-[#1e293b]">
          <span style={{ fontSize: 9, color: '#64748b', fontWeight: 800, letterSpacing: '0.05em' }}>OVERALL RISK</span>
  ...
  return (
    <div className="flex items-center justify-between px-3 py-2 rounded-md" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.03)' }}>
      ...
      <div className="flex items-center gap-2">
        <span style={{ fontSize: 11, fontWeight: 800, color: '#f8fafc' }}>{value}</span>
...
function BarrierViz({ upperContrast, lowerContrast, pnet }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 8, color: '#64748b', fontWeight: 800, letterSpacing: '0.05em', marginBottom: 8, textTransform: 'uppercase' }}>Barrier vs Net Pressure</div>
  ...
  return (
    <div className="mb-2">
      <div className="flex justify-between mb-1">
        <span style={{ fontSize: 9, color: isPnet ? color : '#64748b', fontWeight: 700 }}>{label}</span>
